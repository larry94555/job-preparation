...
3. `technically_correct` — Are the claims correct, with **no** false statements (e.g. does not claim
   KV is a fixed per-model constant, that weights grow with request load, that paging is lossy
   compression, or that swap changes the model's output)?
4. `concrete` — Is it specific (names real mechanisms — per-token KV bytes, linear-in-sequence-length
   scaling, internal/external fragmentation, fixed-size blocks, block table, prefix sharing,
   preemption, recompute vs. swap, PCIe/host bandwidth) rather than vague?
