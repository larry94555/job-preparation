---
id: eval-adaptation-essay
applies_to: essay
output_schema: EssayCheckScore
model_hint: "qwen2.5-3b-instruct, temperature 0, response_format=json"
---

# Grading an adaptation-strategy essay

Grade the answer against the question's reference key (`reference_points`). Reward correct, specific
reasoning about how to adapt a model — in-context learning, RAG, fine-tuning, and distillation — and
about the decision axes (freshness, behavior change, cost, latency, attribution) and their
antipatterns. Reward substance, **not** length or eloquence. Answer each check independently as
...
3. `technically_correct` — Are the claims correct, with **no** false statements (e.g. does not claim
   fine-tuning is the right way to hold constantly-changing facts, that RAG changes model behavior or
   output format, or that RAG retrains the model on every query)?
4. `concrete` — Is it specific (names real mechanisms — ICL/few-shot, retrieval/index, weights/LoRA,
   teacher→student, freshness, attribution, training vs. inference cost, latency) rather than vague?
