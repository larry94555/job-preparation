---
id: eval-observability-essay
applies_to: essay
output_schema: EssayCheckScore
model_hint: "qwen2.5-3b-instruct, temperature 0, response_format=json"
---

# Grading an LLM-observability essay

Grade the answer against the question's reference key (`reference_points`). Reward correct, specific
reasoning about observability for LLM systems — traces/spans and correlation IDs, the core signals
(tokens, latency TTFT/TPOT, errors/retries, cost), drift detection and canaries, and capture/replay
with privacy redaction — **not** length or eloquence. Answer each check independently as
...
3. `technically_correct` — Are the claims correct, with **no** false statements (e.g. does not claim
   a span is the whole request, that TTFT measures per-token decode time, that a canary sends traffic
   to no real users, or that logging raw PII verbatim is fine)?
4. `concrete` — Is it specific (names real mechanisms — trace/span, correlation ID, TTFT/TPOT,
   tokens, cost rollup, canary, drift, eval-score trend, redaction/sampling) rather than vague?
