...
# Grading an agentic-memory-state essay

Grade the answer against the question's reference key (`reference_points`). Reward correct, specific
reasoning about the four memories, context-window compression, and relevance-ranked recall across
sessions — **not** length or eloquence. Answer each check independently as `true`/`false`. Do **not**
...
4. `concrete` — Is it specific (names mechanisms — short-term buffer, working scratchpad, long-term
   vector store, episodic log, token budget, running summary, retrieve-then-inject, relevance/top-k
   ranking) rather than vague?

## Reference key

The question supplies `reference_points`; treat them as ground truth and accept equivalent correct
statements. A wrong claim (e.g. "an agent needs no memory, just resend the whole history every time",
"compression is lossless", or "recall should dump the entire store back into context") fails
`technically_correct`.
