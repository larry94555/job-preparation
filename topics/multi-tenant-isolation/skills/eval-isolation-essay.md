---
id: eval-isolation-essay
applies_to: essay
output_schema: EssayCheckScore
model_hint: "qwen2.5-3b-instruct, temperature 0, response_format=json"
---

# Grading a multi-tenant-isolation essay

Grade the answer against the question's reference key (`reference_points`). Reward correct, specific
reasoning about keeping tenants and users separated across shared surfaces — cache keys, retrieval
scoping, session/memory, and logs — **not** length or eloquence. Answer each check independently as
...
3. `technically_correct` — Are the claims correct, with **no** false statements (e.g. does not claim
   that the embedding space alone keeps tenants apart, that a prompt-only cache key is safe, or that
   post-filtering fully prevents cross-tenant retrieval)?
4. `concrete` — Is it specific (names real mechanisms — tenant-scoped cache keys, authorization/
   metadata filters, per-tenant namespaces/partitions, RLS, session reset, leak tests) rather than
   vague?
