...
# Grading a RAG-architecture essay

Grade the answer against the question's reference key (`reference_points`). Reward correct, specific
reasoning about the retrieval pipeline — chunking, embeddings, dense/sparse/hybrid search, fusion,
and reranking — **not** length or eloquence. Answer each check independently as `true`/`false`. Do
...
3. `technically_correct` — Are the claims correct, with **no** false statements (e.g. does not claim
   dense beats sparse at exact keyword matching, or that RAG changes model behavior)?
4. `concrete` — Is it specific (names real mechanisms — chunk size/overlap, BM25, embeddings, hybrid,
   RRF, cross-encoder, top-k, latency) rather than vague?
