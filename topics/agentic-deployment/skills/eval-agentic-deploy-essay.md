...
# Grading an agentic-deployment essay

Grade the answer against the question's reference key (`reference_points`). Reward correct, specific
reasoning about turning an agent into a service — the async job API, never blocking the request thread,
polling for the result, per-user rate limiting, canary rollout, and rollback — **not** length or
...
4. `concrete` — Is it specific (names mechanisms — submit/job id/poll, status queued→running→done,
   token bucket, canary slice, one-command rollback) rather than vague?

## Reference key

The question supplies `reference_points`; treat them as ground truth and accept equivalent correct
statements. A wrong claim (e.g. "the handler should run the agent inline and return the result", "a
canary means deploying to everyone at once", or "roll forward with a fix instead of rolling back during
an incident") fails `technically_correct`.
