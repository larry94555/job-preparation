...
//   node utils/gen-api-key.mjs --write         # save as LLAMA_API_KEY in the secrets file
...
if (doWrite) {
  ...
  const entry = `LLAMA_API_KEY=${key}`;
  const idx = existing.findIndex((l) => /^\s*LLAMA_API_KEY\s*=/.test(l));
  if (idx >= 0) existing[idx] = entry;
  else existing.push(entry);
  while (existing.length && existing[existing.length - 1].trim() === "") existing.pop();
  writeFileSync(path, `${existing.join("\n")}\n`);
  process.stderr.write(`Saved LLAMA_API_KEY to ${path}\n`);
  ...
  process.stderr.write(`  2) Give it to the grader via the secrets file:  LLAMA_API_KEY=${key}\n`);
