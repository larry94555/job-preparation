...
Sensitive values (`LLM_API_KEY`, `AUTH_SECRET`, `DATABASE_URL`, …) come from
...
# then fill in values, e.g.  LLM_API_KEY=...  LLM_BASE_URL=http://165.1.79.248:8080/v1
```
...
#     loaded into env: LLM_API_KEY, LLM_BASE_URL
```
...
`LLM_API_KEY`/`LLM_BASE_URL` with no manual `export`.
...
node utils/gen-api-key.mjs --write        # save it as LLM_API_KEY in secrets/secrets.env
```

`--write` upserts `LLM_API_KEY` in the secrets file (creating it if needed),
...
#    LLM_BASE_URL=http://165.1.79.248:8080/v1

# 4) run an evaluation pass — secrets load automatically
npm run eval-gate
```

If the server is secured but `LLM_API_KEY` is missing/wrong, calls get HTTP
